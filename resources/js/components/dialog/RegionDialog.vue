...
const add = function () {
    axios
        ...
        .then(() => {
            modelChangesStore.addRegion(formData.name);
            close(true, "add");
        })
        .catch((error) => {
            clearFields(formDataErrors);
...
const edit = function () {
    clearFields(formData);
    axios
        .get(`/api/regions/${props.editId}/edit`, formData)
        .then((response) => {
...
const update = function (id) {
    axios
        .patch(`/api/regions/${id}`, formData)
        .then(() => {
            modelChangesStore.editRegion(formData.name);
            close(true, "edit");
        })
        .catch((error) => {
            clearFields(formDataErrors);
