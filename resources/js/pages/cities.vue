...
const headers = computed(() => {
    const baseHeaders = [
        ...
        { title: t("cities.region"), key: "region.name" },
        { title: t("cities.ip_start"), key: "ip_start" },
        { title: t("cities.ip_end"), key: "ip_end" },
...
const closeDialog = function (dataChanged, method) {
    ...
    if (method === "add") {
        showSnackBar(
            t("cities.city") +
                " " +
                modelChangesStore.getCity.lastAdd +
                " " +
                t("cities.was_append"),
            "success"
        );
        ...
        showSnackBar(
            t("cities.city") +
                " " +
                modelChangesStore.getCity.lastEdit +
                " " +
                t("cities.was_edited"),
            "warning"
        );
...
const askToDeleteRow = function (id, name) {
    ...
    alertAcceptText.value = `${t("cities.delete")} ${name}?`;
    idToDelete.value = id;
    modelChangesStore.deleteCity(name);
};
    ...
    axios
        ...
        .then(() => {
            ...
            showSnackBar(
                t("cities.city") +
                    " " +
                    modelChangesStore.getCity.lastDelete +
                    " " +
                    t("cities.was_deleted"),
                "error"
            );
        })
        .catch((error) => {
            ...
            if (error.response.status === 409) {
                showAlertDialog.value = true;
                alertText.value = t("cities.unable_to_delete");
